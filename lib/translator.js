...
function validateTranslatedContent(content) {
    // Check for malformed HTML attributes where LLM may have broken the attribute syntax
    // This pattern detects cases like `<div class">` where an attribute name is followed
    // directly by a quote without the equals sign and value
 ...
 * Escapes {% sequences that are NOT followed by a valid tag name pattern.
 * Valid Hexo tags look like: {% tagname or {% /tagname
 * Invalid/malformed sequences get converted to HTML entities to prevent Hexo parse errors.
 * @param {string} content - Content to sanitize
 * @returns {string} - Sanitized content
 */
function sanitizeHexoTags(content) {
    // Negative lookahead: keep {% only if followed by optional / and a word character
    // This preserves valid tags like {% note %}, {% /tabs %} while escaping broken ones
